...
function toggleSettings() {
  ...
  const foldIcon = document.querySelector('.fold-icon');
...
class Uploader {
  ...
  constructor(file, pathParts) {
    ...
    this.chunkSize = 1024 * 1024 * (parseInt(chunkSizeInput.value) || 10); // Convert MB to bytes, fallback to 10MB
    this.aborted = false;
  }
  ...
  <tr id="upload${idx}" class="uploader">
    ...
    this.$uploadStatus.innerHTML = 'Checking...';
  ...
  async checkExistingFile() {
    try {
      ...
      if (res.status === 200) {
        ...
        const serverFileSize = contentLength ? parseInt(contentLength) : 0;

        // If sizes match exactly, silently overwrite
        // Inside the checkExistingFile method, replace this:
        if (serverFileSize === this.file.size) {
          const choice = confirm(
              `File "${this.file.name}" already exists with the same size (${formatFileSize(serverFileSize).join(" ")}).\n\n` +
          ...
          if (choice) {
            ...
            this.uploadOffset = 0;
            this.$uploadStatus.innerHTML = `Overwriting existing file...`;
          } else {
            // User chose to cancel
            this.aborted = true;
            this.fail("Upload canceled");
            return;
          }
        }
        // Different size - ask user what to do
        else {
          const choice = confirm(
              `File "${this.file.name}" already exists (${formatFileSize(serverFileSize).join(" ")}) ` +
              `but has different size than local file (${formatFileSize(this.file.size).join(" ")}).\n\n` +
              `Click OK to resume upload from ${formatFileSize(serverFileSize).join(" ")},\n` +
              `or Cancel to overwrite the entire file.`
          );

          if (choice) {
            // User chose to resume
            this.uploadOffset = serverFileSize;
            this.$uploadStatus.innerHTML = `Resuming from ${formatFileSize(this.uploadOffset).join(" ")}...`;
          } else {
            // User chose to overwrite
            this.uploadOffset = 0;
            this.$uploadStatus.innerHTML = `Overwriting existing file...`;
  ...
  async uploadFile() {
    ...
    try {
      if (this.uploadOffset === 0) {
        // Start a new upload with the first chunk
        await this.uploadChunk(true);
      } else {
        // Resume upload from where we left off
        await this.uploadChunk(false);
      }
    } catch (error) {
  ...
  async uploadChunk(isFirstChunk) {
    if (this.aborted) return;

    const { file, url, chunkSize, uploadOffset } = this;

    // If we've uploaded everything, we're done
    if (uploadOffset >= file.size) {
      this.complete();
      return;
    }

    const end = Math.min(uploadOffset + chunkSize, file.size);
    const chunk = file.slice(uploadOffset, end);
    const isLastChunk = end === file.size;

    try {
      const xhr = new XMLHttpRequest();
      xhr.open(isFirstChunk ? "PUT" : "PATCH", url);
      ...
      xhr.upload.addEventListener("progress", e => {
        this.progress(e, uploadOffset);
      });

      xhr.addEventListener("load", () => {
        if (xhr.status >= 200 && xhr.status < 300) {
          this.uploadOffset = end;
          ...
          this.fail(`${xhr.status} ${xhr.statusText}`);
        }
      });

      xhr.addEventListener("error", () => this.fail("Network error"), false);
      xhr.addEventListener("abort", () => {
        this.aborted = true;
        this.fail("Upload aborted");
      }, false);

      xhr.send(chunk);

      if (isFirstChunk) {
        this.$uploadStatus.innerHTML = `Starting upload...`;
      } else {
        this.$uploadStatus.innerHTML = `Uploading chunk: ${formatFileSize(uploadOffset).join(" ")} of ${formatFileSize(file.size).join(" ")}...`;
      }
    } catch (error) {
      this.fail(error.message);
    }
  ...
  progress(event, chunkOffset) {
    const now = Date.now();
    const speed = (event.loaded - this.uploaded) / (now - this.lastUptime) * 1000;
    const [speedValue, speedUnit] = formatFileSize(speed);
    const speedText = `${speedValue} ${speedUnit}/s`;

    const totalUploaded = chunkOffset + event.loaded;
    const progress = formatPercent((totalUploaded / this.file.size) * 100);
    const duration = formatDuration((this.file.size - totalUploaded) / speed);
  ...
  async retry() {
    this.aborted = false;
    this.$uploadStatus.innerHTML = `Checking file status...`;
    await this.checkExistingFile();
  }

  ajax() {
    // Begin by checking for existing file
